...
If Rule AI Quality shows no recent attempts after a draft/probe run, check app logs for `rule_ai_draft_audit_failed` or `rule_ai_golden_probe_audit_failed`, then run `python3 scripts/mailagent_preflight.py` and confirm `mail_rule_ai_draft_audit` and `mail_rule_ai_golden_probe_runs` exist. Audit is best-effort observability, so draft/probe responses may still succeed even when audit persistence fails. Do not retry by saving rules; Save Rule is still a separate human action.
